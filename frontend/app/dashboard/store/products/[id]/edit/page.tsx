...
export default function EditProductPage() {
  ...
  const handleAddVariant = () => {
    if (formData.variantDimension) {
      setFormData({ ...formData, variants: [...formData.variants, { name: '', price: '', inventory: null }] });
    } else {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <Link href="/dashboard/store/products" className="inline-flex items-center gap-2 text-text-secondary hover:text-text-primary mb-6">
      ...
      <div className="bg-surface-raised rounded-lg border border-border-default p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input type="text" required value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} className="w-full px-4 py-2 bg-surface-base border border-border-default rounded-lg text-text-primary focus:outline-none focus:ring-2 focus:ring-purple-500" />
          </div>

          <div>
            <label className="block text-sm font-medium text-text-primary mb-2">Description</label>
            <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} rows={4} className="w-full px-4 py-2 bg-surface-base border border-border-default rounded-lg text-text-primary focus:outline-none focus:ring-2 focus:ring-purple-500" />
          </div>

          <div>
            <label className="block text-sm font-medium text-text-primary mb-2">Base Price ({formatCurrency(0)}) *</label>
            <input type="number" required step="0.01" min="0" value={formData.basePrice} onChange={(e) => setFormData({ ...formData, basePrice: e.target.value })} className="w-full px-4 py-2 bg-surface-base border border-border-default rounded-lg text-text-primary focus:outline-none focus:ring-2 focus:ring-purple-500" />
          </div>
          ...
          <div>
            ...
            <input type="text" value={formData.variantDimension} onChange={(e) => setFormData({ ...formData, variantDimension: e.target.value })} className="w-full px-4 py-2 bg-surface-base border border-border-default rounded-lg text-text-primary focus:outline-none focus:ring-2 focus:ring-purple-500" placeholder="e.g., Size, Color" />
          </div>
