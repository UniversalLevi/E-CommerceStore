...
import { Sparkles, Download, Save, Upload } from 'lucide-react';
...
export default function FacebookAdsPage() {
  ...
  useEffect(() => {
    if (currentDraft && currentDraft.platform === 'facebook') {
      setFormData({
  ...
  const generateInterests = async () => {
    ...
    try {
      const response = await fetch('/api/facebook/generate-interests', {
        method: 'POST',
  ...
  const generateCaptions = async () => {
    ...
    try {
      const response = await fetch('/api/facebook/generate-captions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ productName: formData.productName }),
      });
  ...
  const generateHashtags = async () => {
    ...
    try {
      const response = await fetch('/api/facebook/generate-hashtags', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ productName: formData.productName }),
      });
  ...
  const generateRecommendations = async () => {
    ...
    try {
      const response = await fetch('/api/facebook/generate-recommendations', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ productName: formData.productName }),
      });
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      const campaignData = {
        ...
        customAudienceFile: formData.customAudienceFile?.name,
      };
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `facebook-campaign-${Date.now()}.json`;
    a.click();
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold text-text-primary">Facebook Ads Builder</h1>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2">
            ...
            <FileUploader
              ...
              onFileSelect={(file) => setFormData({ ...formData, customAudienceFile: file })}
              currentFile={formData.customAudienceFile}
            />
          </div>
        </div>
      </section>
